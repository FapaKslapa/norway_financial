...
export function RecentTransactionsCard({
  ...
  return (
    <Card className="border border-(--card-border) bg-(--card) shadow-(--card-shadow) p-5 apple-widget flex flex-col transition-all h-full">
      <div className="flex flex-row justify-between items-center pb-4 border-b border-(--card-border) mb-4 shrink-0">
      ...
      <div className="overflow-y-auto flex flex-col gap-2 pr-1 scrollbar-thin flex-1 min-h-0">
        {expenses.map((tx) => {
          ...
          return (
            <div
              key={tx.id}
              className="flex justify-between items-center p-2.5 rounded-2xl bg-neutral-500/5 border border-(--card-border)"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  {tx.sharedInfo && (
                    <span className="text-[8px] text-(--text-muted) font-normal mt-0.5 truncate">
                      {tx.sharedInfo.isBorrowed
              ...
              <span
                className={`text-xs font-black shrink-0 ${isExpense ? "text-foreground" : "text-emerald-500"}`}
              >
                {isExpense ? "-" : "+"}{" "}
