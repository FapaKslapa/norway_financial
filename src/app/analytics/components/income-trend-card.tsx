    ...
    <Card className="border border-[var(--card-border)] bg-[var(--card-solid)] shadow-xl p-6 rounded-[2rem] select-none w-full h-full flex flex-col">
      ...
      <CardContent className="p-0 relative flex-1 min-h-0">
        ...
        >
          ...
          {areaD && <path d={areaD} fill="url(#incomeGradient)" stroke="none" />}
          {lineD && <path d={lineD} fill="none" stroke="#34c759" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />}

          {points.map((p, idx) => (
            <circle key={p.label} cx={p.x} cy={p.y}
              r={hoveredIdx === idx ? 5 : 3.5}
              fill="#34c759"
              stroke={hoveredIdx === idx ? "rgba(52, 199, 89, 0.3)" : "rgba(255,255,255,1)"}
